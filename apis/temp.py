...
from utils.WebHookApis import webHookWX
current_directory = os.path.dirname(os.path.dirname(__file__))
log_name = os.path.join(current_directory, 'main.log')
logger.add(log_name, encoding="utf-8", enqueue=True, catch=True, rotation="500MB", retention="10 days", mode="w")
...
class HHUAppointmentApis():
    ...
    def appoint_main(self, user, site_name='江宁校区乒乓球馆', webHookUrl=None):
        username = user['username']
        password = user['password']
        my_favorite_time = user['favorite_time_input']
        my_favorite_no = user['favorite_no_input']
        try:
        ...
        while not self.isAppoint:
            if self.quitTaskFlag():
                content = f'{self.name} 超时退出'
                logger.error(content)
                if webHookUrl:
                    webHookWX(webHookUrl, content)
                return
            ...
            logger.info(f' {self.name} {site_name} 所有的可预约时间段')
            ...
            for slot in all_slot:
                ...
                logger.info(f'{self.name} 正在查看 {resourceName} 的 {slot["startTime"]}-{slot["endTime"]}')
                if slot['status'] == 0:
                    ...
                    logger.info(f'{self.name} {resourceName} 的 {slot["startTime"]}-{slot["endTime"]} 预约结果 {json.dumps(appointment_res, ensure_ascii=False)}')
                    if '最大预约次数' in appointment_res['message']:
                        content = f'{self.name} 预约失败 {resourceName} 的 {slot["startTime"]}-{slot["endTime"]}, {appointment_res["message"]}'
                        logger.error(content)
                        self.isAppoint = False
                        if webHookUrl:
                            webHookWX(webHookUrl, content)
                    ...
                    if appointment_res['status'] == 0 and appointment_res['message'] == 'success':
                        content = f'{self.name} 预约成功 {resourceName} 的 {slot["startTime"]}-{slot["endTime"]}'
                        logger.info(content)
                        self.isAppoint = True
                        if webHookUrl:
                            webHookWX(webHookUrl, content)
...
def main(users, site_name, webHookUrl=None, run_thread=False):
    if run_thread:
        with ThreadPoolExecutor(max_workers=5) as executor:
            executor.map(lambda user: HHUAppointmentApis().appoint_main(user, site_name, webHookUrl), users)
    else:
        for user in users:
            hhuAppointmentApis = HHUAppointmentApis()
            hhuAppointmentApis.appoint_main(user, site_name, webHookUrl)
...
if __name__ == '__main__':
    ...
    users = [
        {
            ...
            'favorite_time_input': ['18:00', '19:00'],
            'favorite_no_input': ['11', '9'],
        },
        {
    ...
    site_name = '江宁校区羽毛球馆'
    run_thread = True
    webHookUrl = "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=2823f309-1e7f-4178-b603-d0fa2f7e9c60"

    # main(users, site_name, my_favorite_time_input, my_favorite_no_input, webHookUrl, run_thread)

    schedule.every().day.at("06:59").do(main, users, site_name, webHookUrl, run_thread)
    while True:
